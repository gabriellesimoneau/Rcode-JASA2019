<<parent_compare_dep30, echo=FALSE, cache=FALSE>>=
knitr::set_parent("C:/Users/Gabrielle/Google Drive/McGill - PhD/Thesis/dw-SM/JASA/Supplementary Material")
@

<<compare_dep30, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE>>=
library(hydroGOF)
...
res_500 <- read.table("X1dep30_500.txt")
res_1000 <- read.table("X1dep30_1000.txt")
@
\begin{table}[H]
\centering
\caption{Summary of the log-survival time distribution under different treatment assignment schemes with 30\% censoring dependent on baseline covariates}
{\tabcolsep=4.25pt{
